...
class ABPTests(unittest.TestCase):
    def test_network_exception_is_preserved(self):
        rule = "@@||Example.COM^$SCRIPT,domain=example.com|~ads.example.com"
        out = merge.normalize_rule(rule)
        self.assertEqual(out, "@@||Example.COM^$domain=example.com|~ads.example.com,script")
    ...
    def test_snippet_filter_is_preserved(self):
        rule = "example.com##+js(set-constant, foo, true)"
        self.assertEqual(merge.normalize_rule(rule), rule)

    def test_regex_with_dollar_is_preserved(self):
        rule = r"/foo\\$bar/$script"
        self.assertEqual(merge.normalize_rule(rule), r"/foo\\$bar/$script")
    ...
    def test_csp_option_with_spaces_is_preserved(self):
